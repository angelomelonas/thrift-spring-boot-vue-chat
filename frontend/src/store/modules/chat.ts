...
import {ChatService, Message, MessageRequest, MessagesRequest, SubscriptionRequest, UnsubscriptionRequest} from "../../../codegen";
...
class ChatModule extends VuexModule {
    ...
    appendMessage(message: Message) {
        if (message) {
            this.messages += "[" + new Date(Date.now()).toISOString() + "] " + message.username + ": " + message.message + "\n";
        } else {
    ...
    startPolling(){
        this.polling = setInterval(() => {
            ...
            this.chatClient.getMessages(messagesRequest).then((messages: Message[]) => {
    ...
    unsubscribe() {
        ...
        this.chatClient.unsubscribe(unsubscriptionRequest).then((message: Message) => {
            if (message.message) {
                if (!message.message.includes("Error")) {
